...
get (x:xs) (Node _ imap) = case M.lookup (ord x) imap of
...
put (x:xs) val (Node v imap)  = case M.lookup (ord x) imap of
                                  Nothing   -> (Nothing, Node v $ M.insert (ord x) (snd $ put xs val empty) imap)
                                  Just dict -> let (oldVal, newDict) = put xs val dict
                                               in  (oldVal, Node v $ M.insert (ord x) newDict imap)
...
remove (x:xs) (Node v imap)  = case M.lookup (ord x) imap of
                                 Nothing   -> (Nothing, Node v imap)
                                 Just dict -> let (val, newDict@(Node _ newImap)) = remove xs dict
                                              in if M.null newImap
                                                 then (val, Node v $ M.delete (ord x) imap)
                                                 else (val, Node v $ M.insert (ord x) newDict imap)
...
prefixFoldd prefix fun initV (Node Nothing imap)    = foldr foldingFun initV keyAssocs
  where
    assocs :: [(Int, Dictionary)]
    assocs = M.toAscList imap

    keyAssocs :: [(String, Dictionary)]
    keyAssocs = over _1 ((prefix <>) . pure . chr) <$> assocs
